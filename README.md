...
其中 `## 2. 故事版` 必须是图片：一张横屏 5 格 16:9 storyboard / contact-sheet 图。

不能只给文字分镜、表格、prompt 或镜头说明。

图片不能是竖屏拼贴或方图思维。每个 panel 都必须是横屏 16:9 构图，默认横向单行排列。
...
```text
用 storyboard-dtc-product-creative，基于这个商品链接输出产品图、横屏 DTC 商品创意故事版图片、15 秒英文脚本。商品必须保持真实，创意不能改变商品。
```

```text
用这个产品图做 DTC 商品创意故事版图片。产品为主，先锁商品真相，再选择一个 DTC creative lens，输出一张横屏 5 格 16:9 连续关键帧图片和英文脚本。
```
